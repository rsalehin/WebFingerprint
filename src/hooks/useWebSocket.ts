...
export function useWebSocket(): UseWebSocketResult {
  ...
  const [revisitData, setRevisitData] = useState<RevisitPayload | null>(null);
  ...
  const handleMessage = useCallback((message: WSMessage) => {
    switch (message.type) {
      ...
      case 'revisit_detected': {
        const payload = message.payload as RevisitPayload;
        setRevisitData(payload);
        break;
      }
