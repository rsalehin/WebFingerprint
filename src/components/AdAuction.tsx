...
export function AdAuction({ visitor }: AdAuctionProps) {
  ...
  useEffect(() => {
    ...
    if (!auctionResult.aiPowered) {
      // Calculated fallback: reveal all bids immediately
      setVisibleCount(bids.length);
          ...
          <>
            ...
            {auctionClosed && (
              <div className="auction-info-box">
                ...
                {visitor?.client?.adBlockerDetected && (
                  <p className="ai-note">
                    <span className="icon-shield"></span> Note: your ad blocker would normally suppress these bids in the wild.
                  </p>
                )}
                <p className="ai-note">
