...
export async function collectClientInfo(): Promise<ClientInfo> {
  ...
  const result: ClientInfo = {
    ...
    // Entropy score (placeholder - will be computed below)
    entropyScore: 0,
  };

  // Generate user profile now that we have all data
  result.userProfile = generateUserProfile(result);
  result.entropyScore = calculateEntropyScore(result);

  return result;
}

function calculateEntropyScore(info: ClientInfo): number {
  let score = 0;

  // Canvas fingerprint present and not generic
  if (info.canvasFingerprint && info.canvasFingerprint !== 'unavailable' && info.canvasFingerprint.length > 4) score += 20;

  // WebGL renderer present
  if (info.webglRenderer) score += 15;

  // WebGPU available
  if (info.webgpuFingerprint?.available === true) score += 15;

  // Audio fingerprint present
  if (info.audioFingerprint && info.audioFingerprint !== 'unavailable' && info.audioFingerprint.length > 4) score += 10;

  // Rich font set
  if ((info.fontsDetected?.length ?? 0) > 20) score += 10;

  // Browser plugins present
  if (typeof navigator !== 'undefined' && navigator.plugins?.length > 0) score += 5;

  // Touch device (adds uniqueness on desktop it won't be set)
  if (info.maxTouchPoints > 0) score += 5;

  // Higher-RAM machine
  if ((info.deviceMemory ?? 0) > 4) score += 5;

  // Non-UTC timezone
  if (info.timezone && info.timezone !== 'UTC' && info.timezone !== 'Etc/UTC') score += 5;

  // Non-default locale
  if (info.language && !info.language.startsWith('en-US')) score += 5;

  // Known connection type
  if (info.connectionType !== null && info.connectionType !== undefined) score += 5;
