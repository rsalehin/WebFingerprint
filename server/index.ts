...
import { generateAIProfile, generateAIAuction, trackUniqueVisitor, getTotalUniqueVisitors, checkAndRecordVisit, type GeoData } from './ai-profiler';
import {
  initSharedVisitors,
  onVisitorEvent,
      ...
      try {
        ...
        if (data.type === 'client_info') {
          ...
          if (visitor) {
            ...
            // Track unique visitor and check for revisit
            if (payload.clientInfo.fingerprintId && payload.clientInfo.crossBrowserId) {
              ...
              // Revisit detection — fire and forget, send WS message if returning visitor
              checkAndRecordVisit(payload.clientInfo.fingerprintId).then((revisit) => {
                if (revisit) {
                  const ws = connections.get(visitorId);
                  if (ws && ws.readyState === WebSocket.OPEN) {
                    ws.send(JSON.stringify({
                      type: 'revisit_detected',
                      payload: { visitCount: revisit.visitCount, firstSeen: revisit.firstSeen } satisfies RevisitPayload,
                    }));
                  }
                }
