 ...
 * Uses Anthropic Claude API to analyze user data and infer profile
 */
...
if (anthropic) {
  console.log('Anthropic Claude initialized (claude-haiku-4-5-20251001)');
} else {
 ...
 */
async function callClaude(systemMessage: string, userPrompt: string, maxTokens: number): Promise<string | null> {
...
export async function generateAIProfile(clientInfo: Partial<ClientInfo>, geo?: GeoData): Promise<{
  ...
  if (text) {
    ...
    if (profile) {
      ...
      (profile as UserProfile & { aiSource: string }).aiSource = 'claude';
  ...
  console.log('Claude AI failed - using rule-based fallback');
  const fallbackProfile = generateFallbackProfile(clientInfo, geo);
  return { profile: fallbackProfile, source: 'fallback', error: 'Claude AI failed' };
}
 ...
 * Uses Claude to generate personalized bids based on user profile
 */
...
export async function generateAIAuction(
  ...
  if (anthropic) {
    ...
    if (text) {
      ...
      if (result) {
        console.log('AI Auction: Claude success');
        await setAuctionCache(cacheKey, result);
        return result;
      }
    }
    console.error('AI Auction: Claude failed or returned unparseable response');
  }
